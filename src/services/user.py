...
class UserService:
    ...
    async def register_user(self, user: UserAuthData, db: AsyncSession) -> UserAuthReturn:
        ...
        user = await self.repository.create_user(user.username, hashed_password, db)

        token = encode_jwt({'id': user.id, "role": user.role})

        return UserAuthReturn(id=user.id, token=token, username=user.username)

    async def login_user(self, user: UserAuthData, db: AsyncSession) -> UserAuthReturn:
        user_from_db = await self.repository.get_user_by_email_or_none(user.username, db)
        ...
        return UserAuthReturn(id=user_from_db.id, token=token, email=user.username)
